...
const {isConnection} = require('graphql-sequelize/lib/relay');
...
module.exports = function generateIncludes(simpleAST, type, context, options, joinType) {
  ...
  return Promise.all(Object.keys(simpleAST.fields).map(function (key) {
    ...
    if (association) {
      includeOptions = argsToFindOptions.default(args, association.target);
      allowedAttributes = Object.keys(association.target.rawAttributes);
      includeOptions.required=joinType === "left" ? false : true;
